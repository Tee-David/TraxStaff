...
export function PlatformRail() {
  ...
  return (
    /* Below `sm` the hero it sits on is a dark stage in both themes (see
       `.mk-hero` in globals.css), so the rule and every label here flip to
       white and back at the same 640px line. */
    <motion.div {...revealStagger()} className="relative z-10 shrink-0 border-t border-white/12 sm:border-ink/10">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-2 px-5 py-[clamp(0.625rem,1.7vh,1.25rem)] sm:flex-row sm:justify-between sm:gap-10 sm:px-8">
        <motion.p
          {...item}
          className="text-center text-xs leading-snug text-white/65 sm:max-w-xs sm:text-left sm:text-sm sm:text-muted"
        >
          ...
          {platforms.map(({ icon: Icon, label, available }) => (
            <motion.div
              ...
              className={`flex shrink-0 items-center gap-1.5 sm:gap-2 ${
                available ? "text-white sm:text-ink" : "text-white/40 sm:text-faint"
              }`}
            >
              <Icon className="h-3.5 w-3.5 sm:h-5 sm:w-5" />
              <span className="text-[11px] font-semibold tracking-tight sm:text-sm">{label}</span>
              {!available && (
                <span className="rounded-full border border-white/25 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.1em] text-white/40 sm:border-border sm:px-2 sm:text-[10px] sm:text-faint">
                  Soon
                </span>
              )}
            </motion.div>
