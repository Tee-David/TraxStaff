...
/**
 * Custom cursor: a spinning bracket that snaps around anything marked
 * `.cursor-target`.
 *
 * Three deliberate departures from the upstream component:
 *
 * 1. It renders nothing until mounted. `isMobile` and the reduced-motion query
 *    both read from `window`, so the server and the first client render would
 *    otherwise disagree about whether the cursor exists at all — a hydration
 *    mismatch.
 * 2. It renders nothing when `prefers-reduced-motion` is set. The cursor spins
 *    forever and replaces the system pointer; that is precisely the kind of
 *    thing that setting exists to switch off, and hiding the real pointer
 *    without providing a stable replacement is worse than not doing it.
 * 3. `mix-blend-mode` is off in the CSS — see the note there — so the cursor
 *    can actually be brand orange rather than the inverse of it.
 */
 ...
 * will-change of those, or contain). When one does, the cursor's translate no
 * longer maps to viewport coordinates, so measure the block and subtract it.
 */
...
export default function TargetCursor({
  ...
  // gsap tweens this ref object's own `current`, which is why it is a ref and
  // not a plain number — the tween needs a stable object to write into.
  ...
  const disabled = !mounted || isMobile || prefersReduced;
